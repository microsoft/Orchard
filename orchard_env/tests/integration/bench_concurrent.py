...
class TimingRecord:
    ...
    sandbox_id: str | None  # sandbox ID (available after creation)
...
class TimingTracker:
    ...
    def print_summary(self):
        ...
        for op in op_order:
            ...
            print(f"  ┌─ {label}")
            print(
                f"  │  Total: {len(records)}  Success: {len(success_records)}  Failed: {len(fail_records)}"
            )
            if stats:
                print("  │  Duration (seconds):")
                print(
                    f"  │    min={stats['min']:.3f}  avg={stats['avg']:.3f}  max={stats['max']:.3f}"
                )
                print(
                    f"  │    P50={stats['p50']:.3f}  P90={stats['p90']:.3f}  P95={stats['p95']:.3f}  P99={stats['p99']:.3f}"
                )
            if fail_records:
        ...
        if all_exec_durations:
            ...
            print("  ┌─ All Exec Combined")
            print(
                f"  │  Total: {all_exec_total}  Success: {all_exec_success}  Failed: {all_exec_total - all_exec_success}"
            )
            print("  │  Duration (seconds):")
            print(
                f"  │    min={stats['min']:.3f}  avg={stats['avg']:.3f}  max={stats['max']:.3f}"
        ...
        if sandbox_totals:
            ...
            print(f"  │  {'─' * (len(op_order) * 10 + 30)}")
            print(
                f"  │  Lifecycle total: min={total_stats['min']:.2f}  avg={total_stats['avg']:.2f}  "
        ...
        if create_success > 0:
            print(
                f"  │  Create: {create_success / total_time:.2f} sandboxes/s ({create_success} in {total_time:.1f}s)"
            )
        if all_exec_success > 0:
            print(
                f"  │  Exec:   {all_exec_success / total_time:.2f} execs/s ({all_exec_success} in {total_time:.1f}s)"
            )
        if delete_success > 0:
            print(
                f"  │  Delete: {delete_success / total_time:.2f} deletes/s ({delete_success} in {total_time:.1f}s)"
            )
        print(f"  └{'─' * 60}")
...
class AsyncConcurrentSandboxTest:
    ...
    async def exec_single_command(
        self, client: AsyncSandboxClient, sandbox_id: str, index: int, command: str
    ) -> dict[str, Any]:
    ...
    async def test_concurrent_exec(
        self, sandbox_ids: list[str], command: str = "echo 'Hello World'"
    ) -> dict[str, Any]:
    ...
    async def test_concurrent_create_and_exec(
        ...
        """
        if phased:
            return await self._test_phased(
                num_sandboxes, image, command, cpu, memory, cleanup
            )
        else:
            return await self._test_interleaved(
                num_sandboxes, image, command, cpu, memory, cleanup
            )
    ...
    async def _test_phased(
        ...
        print(f"\n{'='*60}")
        print("Full test (phased): Create -> Exec -> Cleanup")
        ...
        async with AsyncSandboxClient(
            ...
            create_recs = await asyncio.gather(
            ...
            print(f"{'─'*40}")
            print(
                f"  Phase 2/3: Exec commands ({len(exec_commands)} cmds/sandbox x {len(sandbox_map)} sandboxes)"
            )
            print(f"{'─'*40}")

            async def do_exec_all(index: int, sandbox_id: str):
                sandbox = await client.get_sandbox(sandbox_id)
                for exec_idx, (cmd, login) in enumerate(
                    zip(exec_commands, exec_login_shell), start=1
                ):
                    ...
                    try:
                        ...
                        rec.success = job_result.succeeded
                        if not rec.success:
                            rec.error = (job_result.stderr or "").strip()[
                                :200
                            ] or f"exit_code={job_result.exit_code}"
                        stdout_preview = job_result.stdout.strip()[:50]
                        print(
                            f"[{index}] exec#{exec_idx}: {stdout_preview}... ({rec.duration:.2f}s)"
                        )
                    except Exception as e:
                        rec.end_time = time.time()
                        rec.duration = rec.end_time - start
            ...
            if cleanup:
                ...
                async def do_delete(index: int, sandbox_id: str):
                    rec = TimingRecord(
                        sandbox_index=index, sandbox_id=sandbox_id, operation="delete"
                    )
                    start = time.time()
                    rec.start_time = start
                    try:
    ...
    async def _test_interleaved(
        ...
        print(f"\n{'='*60}")
        print("Full test (interleaved): Create + Exec + Cleanup")
        print(f"Sandboxes: {num_sandboxes}")
        print(f"Image: {image}")
        print(f"Command: {command}")
        print(f"{'='*60}\n")
        ...
        async with AsyncSandboxClient(
            ...
            async def create_exec_cleanup(index: int):
                ...
                try:
                    ...
                    print(
                        f"[{index}] Created: {sandbox.sandbox_id} ({rec_create.duration:.2f}s)"
                    )
                    ...
                    for exec_idx, (cmd, login) in enumerate(
                        ...
                        if not rec_exec.success:
                            rec_exec.error = (job_result.stderr or "").strip()[
                                :200
                            ] or f"exit_code={job_result.exit_code}"
                        ...
                        print(
                            f"[{index}] Exec#{exec_idx} done: {result['stdout'][:50]}... ({rec_exec.duration:.2f}s)"
                        )
        ...
        stats = {
            ...
            "cleanup_success": (
                sum(1 for r in results if r["cleanup_success"]) if cleanup else None
            ),
            "overall_time": overall_time,
    ...
    def _print_statistics(self, stats: dict[str, Any]):
        ...
        if stats["successful"] > 0:
            ...
            print(
                f"  Throughput: {stats['successful'] / stats['overall_time']:.2f} sandboxes/s"
            )
...
async def main():
    ...
    parser.add_argument(
        "--image",
        default="mirror.gcr.io/swebench/sweb.eval.x86_64.astropy_1776_astropy-12907",
        help="Docker image",
    )
    ...
    parser.add_argument(
        "--test-type",
        choices=["create", "exec", "full"],
        default="full",
        help="Test type: create=create only, exec=exec only, full=full lifecycle",
    )
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Don't cleanup sandboxes"
    )
    parser.add_argument(
        "--phased",
        action="store_true",
    ...
    parser.add_argument(
        "--command",
        default="python -c 'import sys; print(f\"Python {sys.version}\")'",
        help="Command to execute",
    )
    parser.add_argument(
        "--api-key",
        default=None,
        help="API key for authentication (or set SANDBOX_API_KEY env var)",
    )
    parser.add_argument(
        "--save-timing", default=None, help="Save detailed timing data to JSON file"
    )

    args = parser.parse_args()

    tester = AsyncConcurrentSandboxTest(
    ...
    try:
        if args.test_type == "create":
            await tester.test_concurrent_create(
