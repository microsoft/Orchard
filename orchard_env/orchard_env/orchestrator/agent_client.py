...
class AgentClient:
    ...
    async def exec_command(
        ...
        try:
            ...
            raise TimeoutError(f"Agent exec timed out after {timeout}s")
    ...
    async def download_file(self, pod_ip: str, path: str) -> dict:
        ...
        async with session.get(
            ...
            if resp.status != 200:
                body = await resp.text()
                raise RuntimeError(
                    f"Agent download failed (HTTP {resp.status}): {body}"
                )
            return await resp.json()
    ...
    async def list_files(self, pod_ip: str, path: str = "/workspace") -> list[dict]:
        ...
        async with session.get(
            ...
            if resp.status != 200:
                body = await resp.text()
                raise RuntimeError(
                    f"Agent list_files failed (HTTP {resp.status}): {body}"
                )
            data = await resp.json()
            return data["files"]
    ...
    def pty_ws_url(self, pod_ip: str) -> str:
        """Return the WS URL for the agent's /exec/pty endpoint."""
        return f"ws://{pod_ip}:{self._agent_port}/exec/pty"

    async def open_pty_ws(self, pod_ip: str) -> aiohttp.ClientWebSocketResponse:
        """Open a WebSocket to the agent's /exec/pty endpoint.

        The caller is responsible for closing the returned WS.  Uses the
        shared aiohttp session so connections to the same pod are pooled.
        """
        session = await self._get_session()
        return await session.ws_connect(
            self.pty_ws_url(pod_ip),
            heartbeat=30.0,
            # Allow large PTY output bursts.
