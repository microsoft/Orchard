...
RESOURCE_GROUP="${RESOURCE_GROUP:-sandboxd16n2-orchestrator-rg}"
LOCATION="${LOCATION:-westus3}"
CLUSTER_NAME="${CLUSTER_NAME:-sandboxd16n2-aks}"
ACR_NAME="${ACR_NAME:-sandboxd16n2acr$(date +%s)}"  # Must be globally unique
LOG_ANALYTICS_WORKSPACE="${LOG_ANALYTICS_WORKSPACE:-sandboxd16n2-logs}"

# Node pool configurations
SYSTEM_NODE_SIZE="${SYSTEM_NODE_SIZE:-Standard_D16ads_v7}"
SYSTEM_NODE_COUNT="${SYSTEM_NODE_COUNT:-1}"
SYSTEM_NODE_MIN="${SYSTEM_NODE_MIN:-1}"
SYSTEM_NODE_MAX="${SYSTEM_NODE_MAX:-1}"

SANDBOX_NODE_SIZE="${SANDBOX_NODE_SIZE:-Standard_D16ads_v7}"
SANDBOX_NODE_MIN="${SANDBOX_NODE_MIN:-1}"
SANDBOX_NODE_MAX="${SANDBOX_NODE_MAX:-50}"
...
run_cmd az group create \
    --name "$RESOURCE_GROUP" \
    --location "$LOCATION" \
    --tags "purpose=sandboxd16n2-orchestrator" "managed-by=script"
...
run_cmd az aks create \
    ...
    --generate-ssh-keys \
    --tags "purpose=sandboxd16n2-orchestrator"
