...
RESOURCE_GROUP="${RESOURCE_GROUP:-sandboxd16-orchestrator-rg}"
LOCATION="${LOCATION:-westus3}"
CLUSTER_NAME="${CLUSTER_NAME:-sandboxd16-aks}"
ACR_NAME="${ACR_NAME:-sandboxd16acr$(date +%s)}"  # Must be globally unique
LOG_ANALYTICS_WORKSPACE="${LOG_ANALYTICS_WORKSPACE:-sandboxd16-logs}"
...
run_cmd az group create \
    --name "$RESOURCE_GROUP" \
    --location "$LOCATION" \
    --tags "purpose=sandboxd16-orchestrator" "managed-by=script"
...
run_cmd az aks create \
    ...
    --generate-ssh-keys \
    --tags "purpose=sandboxd16-orchestrator"
