...
class K8sClient:
    ...
    async def create_pod(
        ...
        # Agent readiness probe — pod is "Ready" only when the in-pod agent
        # responds to /health.  This ensures the orchestrator can immediately
        # start sending exec / file requests as soon as PodWatcher sees Ready.
        ...
        container = client.V1Container(
            ...
            command=command or [
                "sh", "-c",
                "/opt/sandbox-agent/start.sh & sleep infinity",
            ],
            resources=resources,
            ...
            volume_mounts=[agent_volume_mount],
        )
    ...
    async def delete_pod(self, name: str, namespace: str, grace_period_seconds: Optional[int] = None) -> None:
        """Delete a pod.
