...
class CreateSandboxRequest(BaseModel):
    ...
    block_network: bool = Field(
        default=True,
        description="Whether to block network egress"
    )
    sandbox_id: Optional[str] = Field(
        default=None,
        description="Optional custom sandbox ID"
    )
...
async def create_sandbox(request: CreateSandboxRequest, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        if any(keyword in err_str for keyword in [
            'Connection timed out', 'Max retries exceeded', 'timed out',
...
async def exec_command(sandbox_id: str, request: ExecRequest, api_key: str = Depends(verify_api_key)):
...
async def wait_job(job_id: str, timeout: int = 300, api_key: str = Depends(verify_api_key)):
...
async def apply_patch(sandbox_id: str, request: ApplyPatchRequest, api_key: str = Depends(verify_api_key)):
...
async def wait_sandbox_ready(sandbox_id: str, timeout: int = 3600, api_key: str = Depends(verify_api_key)):
...
async def upload_file(sandbox_id: str, request: UploadFileRequest, api_key: str = Depends(verify_api_key)):
    ...
    if not sandbox.ready:
        raise HTTPException(status_code=400, detail=f"Sandbox {sandbox_id} is not ready")
    ...
    try:
        ...
        if not pod_ip:
            raise HTTPException(status_code=503, detail=f"No pod IP available for sandbox {sandbox_id}")
        
        result = await agent_client.upload_file(
            pod_ip=pod_ip,
            path=request.path,
...
async def download_file(sandbox_id: str, path: str, api_key: str = Depends(verify_api_key)):
...
async def list_files(sandbox_id: str, path: str = "/workspace", api_key: str = Depends(verify_api_key)):
